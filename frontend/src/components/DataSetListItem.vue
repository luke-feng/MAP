      ...
      <md-card-expand>
        <md-card-actions md-alignment="space-between">
          <div>
            ...
            <md-button v-if="dataset.status === 'analysed'" @click="showFileShare = true">Share</md-button>
            <md-button class="md-accent" v-else @click="notifyNotAnalysed">Share</md-button>
            ...
            <!-- Form -->
            <el-dialog :visible.sync="showFileShare" append-to-body=true title="Share More Information" width=30% close-on-click-modal=false>
              <file-share-form :dataset="dataset" :showFileShare="showFileShare" v-on:stateChanged="updateFileShare">
              </file-share-form>
            </el-dialog>
          </div>
...
export default {
  ...
  computed: {
    ...
    shareAllowed: function () {
      return this.fileSize < 50000000
    }
  },
  components: {
    'file-share-form': FileShareForm
  },
  methods: {
    updateFileShare: function () {
      this.closeShareForm()
      return this.showFileShare
    },
    closeShareForm: function () {
      this.showFileShare = false
    },
